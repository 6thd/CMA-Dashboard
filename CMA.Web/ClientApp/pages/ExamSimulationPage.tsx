...
// Types for AI analysis and study plan
...
interface AnalysisResult {
  weakAreas: AnalysisArea[];
  strongAreas: AnalysisArea[];
...
const ExamSimulationPage: React.FC = () => {
  ...
  const questionStartTimeRef = useRef<number>(Date.now()); // Timer effect
  useEffect(() => {
  ...
  const handleStartExam = () => {
    ...
    if (settings.adaptiveMode) {
      ...
      examQuestions = AIExamService.generateAdaptiveExam(
        settings.studentId,
      ...
      if (settings.selectedSections.length === 0) {
        ...
        filteredQuestions = questionsBank.filter(
          q => q.sectionId && settings.selectedSections.includes(q.sectionId)
        );
      }
  ...
  if (step === 'setup') {
    return (
      <div className="fade-in">
        <h1 className="text-3xl font-bold text-white text-shadow mb-6">
          CMA Part 1 Exam Simulation
        </h1>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
              <div className="bg-white/10 p-4 rounded-lg text-center">
                <div className="text-3xl font-bold text-white">
                  {aiAnalysis.overallPerformance.toFixed(1)}%
                </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div>
                ...
                {aiAnalysis.weakAreas.length > 0 ? (
                  <div className="space-y-3">
                    {aiAnalysis.weakAreas.map((area: AnalysisArea, index: number) => (
                      <div
                        key={index}
                      ...
                      >
                        <div className="flex justify-between">
                          ...
                          <span className="text-red-400">
                            {(area.performance * 100).toFixed(1)}%
                          </span>
                        </div>
                      </div>
                    ))}
              ...
              <div>
                ...
                {aiAnalysis.strongAreas.length > 0 ? (
                  <div className="space-y-3">
                    {aiAnalysis.strongAreas.map((area: AnalysisArea, index: number) => (
                      <div
                        key={index}
          ...
          <div className="bg-glass backdrop-blur-lg border border-white/20 rounded-lg shadow-xl p-6">
            ...
            <div className="space-y-3">
              {aiAnalysis.recommendations.map((rec: string, index: number) => (
                <div
                  key={index}
                  className="p-3 bg-blue-500/10 border border-blue-500/30 rounded text-sm"
                >
                  {rec}
                </div>
              ))}
          ...
          <div className="bg-glass backdrop-blur-lg border border-white/20 rounded-lg shadow-xl p-6 mb-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div>
                ...
                {studyPlan.focusAreas.map((area: StudyFocusArea, index: number) => (
                  <div
                    key={index}
                    className="p-4 bg-red-500/10 border border-red-500/30 rounded-lg mb-3"
                  >
                    ...
                    <div className="text-sm text-gray-300">
                      Recommended: {area.recommendedHours} hours
                    </div>
                    <div className="mt-2">
                      {area.topics.map((topic: string, topicIndex: number) => (
                        <span
                          key={topicIndex}
                          className="inline-block bg-white/10 text-xs px-2 py-1 rounded mr-2 mb-1"
                        >
                          {topic}
                        </span>
                      ))}
              ...
              <div>
                ...
                {studyPlan.reviewAreas.map((area: StudyFocusArea, index: number) => (
                  <div
                    key={index}
                    className="p-4 bg-green-500/10 border border-green-500/30 rounded-lg mb-3"
                  >
                    <div className="font-medium text-white">{area.title}</div>
                    <div className="text-sm text-gray-300 mt-1">Priority: {area.priority}</div>
                    <div className="text-sm text-gray-300">
  ...
  if (step === 'results') {
    ...
    const averageTimePerQuestion =
      answeredQuestions.length > 0 ? Math.round(totalTimeSpent / answeredQuestions.length) : 0;
            ...
            <div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-2">
                {correctAnswers.map((q, index) => (
                  <div
                    key={index}
                    className="p-3 bg-green-500/10 border border-green-500/30 rounded text-sm"
                  >
            ...
            <div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-2">
                {incorrectAnswers.map((q, index) => (
                  <div
                    key={index}
                    className="p-3 bg-red-500/10 border border-red-500/30 rounded text-sm"
                  >
                    Q{questions.indexOf(q) + 1}: {q.question_text.substring(0, 50)}...
                  </div>
                ))}
