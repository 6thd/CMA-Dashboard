...
const QuestionsPage: React.FC = () => {
  ...
  const handleGenerateQuestions = async () => {
    ...
    try {
      ...
      // Placeholder while AI generation is disabled
      ...
      logService.addLog(
        ...
        `Generated ${aiCount} questions for topic "${aiTopic}" in section "${section?.title}"`
      );
    ...
    <div>
      <div className="flex flex-col md:flex-row justify-between items-center mb-6 gap-4">
        ...
        <button
          ...
          title={
            isModerator
              ? "You don't have permission to generate questions"
              : !selectedSectionId
                ? 'Please select a unit and section first'
      ...
      <div className="bg-glass backdrop-blur-lg border border-white/20 shadow-xl rounded-lg p-4 mb-6">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
          ...
          <select
            ...
            className="w-full pl-3 pr-10 py-2 text-base bg-white/10 border-white/30 focus:outline-none focus:ring-primary focus:border-primary sm:text-sm rounded-md text-white"
          ...
          <select
            ...
            className="w-full pl-3 pr-10 py-2 text-base bg-white/10 border-white/30 focus:outline-none focus:ring-primary focus:border-primary sm:text-sm rounded-md text-white disabled:bg-gray-500/20"
          ...
          <select
            ...
            onChange={e => setDifficultyFilter(e.target.value as Question['difficulty'] | '')}
            className="w-full pl-3 pr-10 py-2 text-base bg-white/10 border-white/30 focus:outline-none focus:ring-primary focus:border-primary sm:text-sm rounded-md text-white"
      ...
      <div className="space-y-4">
        {filteredQuestions.length > 0 ? (
          filteredQuestions.map(q => (
            <div
              key={q.id}
              className="bg-glass backdrop-blur-lg border border-white/20 p-4 rounded-lg shadow-lg"
            >
              ...
              {q.options && (
                <div className="mt-2 grid grid-cols-1 sm:grid-cols-2 gap-2 text-sm text-gray-200">
                  {Object.entries(q.options).map(([key, value]) => (
                    <p
                      key={key}
                      className={key === q.correct_answer ? 'text-green-300 font-bold' : ''}
                    >
                      {key}: {value}
                    </p>
                  ))}
        ...
        <div className="space-y-4">
          <div>
            ...
            <input
              type="text"
              value={
                syllabusData.flatMap(u => u.sections).find(s => s.id === selectedSectionId)
                  ?.title || ''
              }
              readOnly
              disabled
          ...
          <div className="flex justify-end pt-2 space-x-3">
            ...
            <button
              ...
              className="px-4 py-2 bg-primary text-white rounded-lg hover:bg-primary-dark disabled:bg-gray-500/50"
            >
