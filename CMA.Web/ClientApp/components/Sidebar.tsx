...
const Sidebar: React.FC<SidebarProps> = ({ isOpen, onClose }) => {
  ...
  useEffect(() => {
    const handleClickOutside = (event: MouseEvent) => {
      ...
      if (isOpen && backdrop && 
          (event.target === backdrop || !sidebar?.contains(event.target as Node))) {
        onClose();
      }
    };
  ...
  return (
    <>
      ...
      <div className="hidden md:flex md:flex-shrink-0">
        <div className="w-64 bg-surface/80 backdrop-blur-xl border-r border-white/10 text-gray-200 flex flex-col">
          <div className="h-20 flex items-center justify-center border-b border-white/10 bg-gradient-primary">
            <div className="flex items-center space-x-3 space-x-reverse">
              ...
              <h1 className="text-xl font-bold text-white text-shadow">CMA STUDY BUDY</h1>
            </div>
          </div>
          ...
          <nav className="flex-1 px-4 py-6 space-y-2 overflow-y-auto">
            {menuItems.map((item) => (
              <Link
                ...
                to={item.path}
                onClick={onClose}
                className={`sidebar-link flex items-center px-3 py-2.5 rounded-lg transition-all duration-200 text-shadow group focus:outline-none focus:ring-2 focus:ring-primary ${
          ...
          <div className="p-4 border-t border-white/10">
            <div className="flex items-center">
              ...
              <div className="mr-3 flex-1 min-w-0">
                ...
                <p className="text-sm text-gray-400 truncate">{currentUser.role}</p>
              </div>
            </div>
          </div>
        </div>
      ...
      {isOpen && (
        <div className="fixed inset-0 z-40 md:hidden">
          ...
          <div 
            id="sidebar"
            className="relative w-64 bg-surface/90 backdrop-blur-xl border-r border-white/10 text-gray-200 flex flex-col h-full transform transition-transform duration-300 ease-in-out"
            role="dialog"
            aria-modal="true"
          ...
          >
            ...
            <button
              onClick={onClose}
              className="absolute top-4 right-4 text-white hover:text-gray-300 focus:outline-none focus:ring-2 focus:ring-primary rounded-full p-1"
